...
<center>
![xx](/endlessriver/img/executor_execution.png)
</center>
...
<!--
<center>
![xx](/endlessriver/img/executor_service_method_table.png)
</center>
...
为了判断线程池的运行状态，给上述线程池状态约定了单调的演化关系：
...
<p style="text-align:center;font-size:.9rem;font-style:italic">线程池执行流程图</sup></p>
...
private boolean addWorker(Runnable firstTask, boolean core) {
    ...
    for (;;) {
         ...
         * 2. 若rs >= shutdown
         *      2.1 rs = shutdown 且firstTask = null 且workQueue为空，继续新建worker
         *      2.2 rs >= stop 不新建worker(return false)
         *      2.3 rs = shutdown，firstTask != null，不新建worker
         *      2.4 rs = shutdown，firstTask = null，workQueue为空，不新建worker
         */
        if (rs >= SHUTDOWN &&
            ! (rs == SHUTDOWN &&
               firstTask == null &&
...
通过上面的分析，例子中的线程在执行完任务后超时被销毁，此时池中没有线程在运行，队列中也没有任务，**那么就意味着所有的逻辑都已经完成，并没有发生阻塞，线程池中的线程数为0，任务队列为空**，但是，线程池的状态还是***RUNNING***！线程池并没有终止，其还可以继续提交任务运行
