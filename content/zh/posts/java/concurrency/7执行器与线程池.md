---
title: "Java并发系列之7——执行器与线程池"
date: 2020-11-03
modDate: 2020-11-03
draft: false
description: "本文详细介绍了Java并发框架中的执行器和线程池，是线程池系列的第一篇。"
tags:
- 执行器
- 线程池
...
Java并发系列的文章到目前为止，虽然没有特别说明，但是使用执行器(Executor(s))的次数已经难以计数了，`Executors`提供了一些非常方便的静态方法，可以根据需要创建不同的`ExecutorService`，然后调用其`execute(Runnable)`或`submit(Callable<T>)`方法。在多线程的条件下，执行器还有一个非常明显的优势，它使用线程池管理线程，避免了系统创建和销毁线程的开销。在一般的Java并发过程中，也建议使用执行器完成任务而非显式地创建线程。
...
{{< img src="/img/executor.png" title="Executor框架组成" width="600px" position="center" >}}

<!--
下图展示了Executor框架的执行逻辑[^2]

<center>
![xx](/img/executor_execution.jpg)
</center>
...
{{<img src="/img/executor_flow.svg" title="线程池执行流程图" width="600px" position="center" >}}
...
<!-- ![线程池添加worker的流程](/img/thread_pool_add_worker.jpg) -->

{{< img  src= "/img/thread_pool_add_worker.jpg" title="线程池添加worker的流程" width="600px" position="center">}}
...
<!-- ![线程池执行任务的流程](/img/thread_pool_run_task.jpg) -->

{{< img src="/img/thread_pool_run_task.jpg" title="线程池执行任务的流程" width="600px" position="center" >}}
