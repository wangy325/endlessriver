...
在[上一篇文章](../1线程与任务)中，虽然创建了多线程，并且线程之间出现了一些**不可预测**的CPU调度，但是由于线程之间是**相互隔离**的——**线程没有访问共同的资源**，尽管在执行任务的过程可能被CPU剥夺运行权，但是当它们再次获得运行权时对运行结果并没有影响，它们是安全的。
