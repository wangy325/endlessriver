...
lastmod: 2020-10-30
draft: false
description: "本文借助两个经典的并发问题阐述了Java并发的死锁问题。"
tags:
- 死锁
- 线程本地存储
...
我们不妨回顾在[转账问题](../资源访问受限-并发基础之二/#2-1-span-id-condition-条件-span)中使用的等待条件——如账户余额不足时使任务等待，在余额足够的时候再进行转账。这个程序没有问题，因为有100个账户每个账户初始金额1000元，而转账金额**不大于**初始金额，所以任一时刻都会有账户的金额满足转账条件。但是如果去除转账金额不大于1000的限制，死锁就会发生，这很容易理解
