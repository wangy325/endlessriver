...
当你在一个服务的方法上使用`@Transactional`注解时，意味着你希望为此方法开启事务支持。如果你的项目成功配置了数据源和事务管理器，Spring会并为此方法使用如下默认设置开启事务：
...
上述代码片段中，**`B`方法的事务会生效，并且和A方法没有关系；而`A`方法是不受事务控制的**。
...
此种情形下，`B`方法实际上被`A`方法的事务所管理：

1. 当`B`抛出异常时：
    - 若`A`方法捕获异常，则`A`和`B`方法均提交；
    - 若`A`方法不捕获异常，则`A`和`B`方法均回滚；
2. 当`A`抛出异常时，`A`和`B`方法均回滚；

若`A`方法捕获了`B`方法抛出异常，则事务提交；若`A`方法没有捕获`B`方法抛出的异常，则认为`A`方法抛出异常，则事务回滚。
