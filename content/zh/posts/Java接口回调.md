...
![iZXAZn.md.png](/img/bs/call-async.png)
<P>
异步调用
...
但是这种方法存在一个问题：如果A()需要知道B()的执行结果(根据业务需求，有些操作如异步线程刷新缓存、推送通知等等不需要，而有些如更改状态的操作则需要)，则需要通过某种方式对B()的执行结果进行**监听**，在Java中可以通过[Future+Callable](../docs/java/concurrency/conecptes/3获取任务的返回值.md)来实现。

### 3. 回调

<center style = "font-size: .8rem; font-style:italic; color: grey">

![iZXmGT.md.png](/img/bs/callback.png)
<p>
回调模式
</center>
 ...
 * 使用场景: A想完成某事, 但A不能独立完成,需要借助B的力量, B完成之后, 
 * 要将结果通知给A(通过调用A的方法实现)
 */
public class Kidd {
    private String name;
...
![iZOfV1.png](/img/bs/full-callback.png)
<p>
完整的回调模式
